...
A self-contained Linux troubleshooting lab. A single Ubuntu 24.04 host is
provisioned into a deliberately broken state that resembles a small internal
...
| Component | Role |
|---|---|
| `lab/lib/common.sh` | Shared constants + helpers. Single source of truth for paths/ports so breakage and validation can never drift. |
| `lab/files`, `lab/services`, `lab/configs` | Healthy templates installed by `setup-lab.sh`. The fixed target equals these. |
| `scripts/setup-lab.sh` | Builds the healthy baseline (idempotent). |
...
| `deploy/install.sh` | One-shot installer: packages → candidate user → setup → break. |
...
```mermaid
flowchart TD
  A[deploy/install.sh] --> B[apt packages]
  B --> C[create candidate user + sudo]
  C --> D[scripts/setup-lab.sh<br/>healthy baseline]
...
|---|---|---|---|---|---|
| A | Web reachable on :80 | nginx `listen` set to 8080; **and** ufw blocks 80 | `ss -ltnp`, `curl`, `ufw status` | `curl :80` refused; ufw active, 80 not allowed | `curl :80` = 200 **and** ufw allows 80 |
...
| C | DNS resolution | `/etc/resolv.conf` points at an unroutable nameserver (`192.0.2.53`) | `resolvectl`, `cat /etc/resolv.conf`, `getent hosts` | name lookups time out | a known hostname resolves |
...
- **Firewall (A):** ufw always permits SSH (port 22) before it is enabled. The
...
SSH configuration is never itself a fault — that would risk lockout. See
`docs/SECURITY.md`.
...
- **Ubuntu only.** The previous attempt targeted AlmaLinux; it is archived on
  the `archived-almalinux-attempt` branch. Supporting both families doubled the
  surface area for little benefit given the stated DigitalOcean target.
